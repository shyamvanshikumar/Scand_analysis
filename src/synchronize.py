...
with rosbag.Bag(output_bag_name, 'w') as outbag:
    ...
    from loc_msgs.msg import Localization2DMsg
    lidar_scan_sub = message_filters.Subscriber('/scan', LaserScan)
    loc_sub = message_filters.Subscriber('/localization', Localization2DMsg)

    # ApproximateTimeSynchronizer wants a list of subscribers
    subscriber_list = [lidar_scan_sub, loc_sub]
    # Customize ApproximateTimeSynchronizer parameters
    ats_queue_size = 100 # Max messages in any queue
    ats_slop = 0.05 # Max delay to allow between messages
